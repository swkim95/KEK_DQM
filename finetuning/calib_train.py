#!/usr/bin/env python3
"""Fine-tuning Script for Calibration Scan Agent (Qwen 2.5 1.5B)"""
...
def finetune_calib_agent(data_path, output_dir):
    logger.info("=" * 70)
    logger.info("🚀 Calibration Scan Agent Fine-tuning")
    logger.info("=" * 70)

    base_model = AGENT_MODELS["calibration"]["base_model"]
...
def main():
    ...
    data_path  = str(script_dir / "finetuning/data/calib_scan_data.json")
    output_dir = str(script_dir / "models/calibration_agent")
    finetune_calib_agent(data_path, output_dir)
