#!/usr/bin/env python3
"""Fine-tuning Script for BrainAgent (Qwen 2.5 1.5B)"""
...
# ──────────────────────────────────────────────
# Weighted Loss Trainer (same as other agents)
# ──────────────────────────────────────────────

class WeightedLossTrainer(Trainer):
    """Assistant decision 토큰에 높은 가중치를 부여하는 Trainer."""
...
def finetune_brain_agent(data_path, output_dir):
    logger.info("=" * 70)
    logger.info("🧠 Brain Agent Fine-tuning")
    logger.info("=" * 70)

    base_model = AGENT_MODELS["brain"]["base_model"]
    ...
    # 9. Save — LoRA adapter + merged full model
...
def main():
    ...
    data_path = str(script_dir / "finetuning/data/brain_data.json")
    output_dir = str(script_dir / "models/brain_agent")
    finetune_brain_agent(data_path, output_dir)
